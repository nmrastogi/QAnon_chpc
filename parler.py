...
def process_data_in_chunks(file_path, chunk_size=50000, start_chunk=1410, partial_file='partial_results_parler.txt', final_file='final_summary_parler.txt'):
    final_category_counts = {category: 0 for category in dictionaries}
    final_token_count = 0
    chunk_count = start_chunk  # Start at the specified chunk number
    total_rows = 0
    ...
    # Iterate over chunks, starting at the specified chunk number
    for chunk in pd.read_csv(file_path, usecols=['body'], chunksize=chunk_size, skiprows=range(1, rows_to_skip + 1)):
        ...
        combined_category_counts_total = chunk_processed['body_category_counts'].apply(pd.Series).sum()
        ...
        chunk_count += 1  # Increment the chunk count for the next iteration
...
# Execute the function from chunk 1410 onward
processed_df = process_data_in_chunks(
    "/uufs/chpc.utah.edu/common/home/u1472278/QAnon/datasets/parler_posts_comments.csv",
    chunk_size=50000,
    start_chunk=1410,
    partial_file='partial_results_parler.txt',
    final_file='final_summary_parler.txt'
)
